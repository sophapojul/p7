...
const recipes = [
    ...
    },
    {
        ...
        "time": 45,
        "description": "Étaler la pâte feuilleté aux dimensions du moule, étaler la moutarde sur la pâte feuilleté, ajouter le thon. Découper les tomates en rondelles et les poser sur le poisson, ajouter un peu de crème fraiche sur toute la tarte et recouvrez de gruyère râpé. Cuire au four 30 minutes",
        "appliance":"Four",
    ...
    },
    {
        ...
        "time": 15,
        "description":"Coupez les fraises en morceaux, découpez la chaire de la pastèque en retirant les pépins. Mettre le tout dans le blender. Ajouter un cuillère à soupe de juste de citron ainsi que les glaçons. Ajoutez quelques feuilles de menthe pour plus de fraicheur. Mixez le tout. Servir et déguster.",
        "appliance": "Blender",
    ...
    },
    {
        ...
        "time": 60,
        "description":"Préparer la frangipane : Mélanger le sucre la poudre d'amande, le beurre et les oeufs. Étaler la moitié de la pate feuilleté et mettre dans un moule à tarte. Garnir de frangipane et recouvrir du reste de pate feuilletée. Mettre au four 30 minutes",
        "appliance": "Four",
        "utensils":["rouleau à pâtisserie","fouet"]
    }
]

// search for ingredients in the filtered recipes;

//search for ingredients in an array of recipes
// function searchIngredients(arr, substring) {
//     if (substring.length < 3) return [];
//     return arr.map(recipe => recipe.ingredients.map(ingredient => ingredient.ingredient)).flat()
// }

// console.log(searchIngredients(recipes, "ane"));

// search for ingredients in an array of recipes with a substring;
function searchIngredients(arr, substring) {
    if (substring.length < 3) {
        return [];
    }
    return Array.from(new Set( arr.map(recipe => recipe.ingredients.map(ingredient => ingredient.ingredient)).flat().filter(ingredient => ingredient.toLowerCase().includes(substring.toLowerCase())) ));
}

function searchIngredientsByLoop(arr, search) {
    const results = [];
    if (search.length < 3) {
        return [];
    }
    for (let i = 0; i < arr.length; i++) {
        for (let j = 0; j < arr[i].ingredients.length; j++) {
            if (arr[i].ingredients[j].ingredient.toLowerCase().includes(search.toLowerCase())) {
                results.push(arr[i]);
            }
        }
    }
...
function searchKeywords(arr, substring, keyword) {
    if (substring.length < 3) return [];
    switch (keyword) {
        case 'ingredients':
            return arr.map(recipe => recipe.ingredients.map(ingredient => ingredient.ingredient)).flat().filter(( ingredient) => ingredient.toLowerCase() ).includes(substring.toLowerCase());
        case 'appareil':
            const results = arr.map(recipe => recipe.appliance).filter(appliance => appliance.toLowerCase().includes(substring.toLowerCase()));
            const obj = {};
        ...
        case 'ustensiles' :
            return arr.map(recipe => recipe.utensils.filter(utensil => utensil.toLowerCase()).includes(substring.toLowerCase()));
            default:
                return [];
    }
