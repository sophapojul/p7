...
const recipes = [
    ...
    },
    {
        ...
        time: 15,
        description:
            'Coupez les fraises en morceaux, découpez la chaire de la pastèque en retirant les pépins. Mettre le tout dans le blender. Ajouter un cuillère à soupe de juste de citron ainsi que les glaçons. Ajoutez quelques feuilles de menthe pour plus de fraicheur. Mixez le tout. Servir et déguster.',
        appliance: 'Blender',
    ...
    },
    {
        ...
        time: 40,
        description:
            'Découper le concombre et les tomates en dés, les mettre dans un saladier avec le maïs et les miettes de poisson, ajouter les pates. Ajouter la mayonnaise. Mélanger le tout et servir frais.',
        appliance: 'Saladier',
    ...
    return Array.from(
        new Set(
            arr
                ...
                .flat()
                .filter((ingredient) =>
                    ingredient.toLowerCase().includes(substring.toLowerCase()),
                ),
        ),
    );
}
    ...
    switch (keyword) {
        ...
        case 'appareil':
            const results = arr
                ...
                .filter((appliance) =>
                    appliance.toLowerCase().includes(substring.toLowerCase()),
                );
            const obj = {};
        ...
        case 'ustensiles':
            return arr.map((recipe) =>
                recipe.utensils
                    .filter((utensil) => utensil.toLowerCase())
                    .includes(substring.toLowerCase()),
            );
        default:
            return [];
...
// return all values of an array of recipes;
function getAllValues(arr, keyword) {
    switch (keyword) {
        case 'ingredients':
            return arr
                .map((recipe) =>
                ...
                .flat();
        case 'appareil':
            return arr.map((recipe) => recipe.appliance);
        case 'ustensiles':
            return arr.map((recipe) => recipe.utensils);
        default:
            return [];
    }
}

// return all values of ingredients and of appliances and of utensils;
function getAllValues1(arr) {
    return arr
        .map((recipe) =>
            recipe.ingredients.map((ingredient) => ingredient.ingredient),
        )
        .flat()
        .concat(arr.map((recipe) => recipe.appliance))
        .concat(arr.map((recipe) => recipe.utensils))
        .flat();
}
